...
function init (client, d3) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    ...
    var loopSpam = ['DEBUG ', 'INTERNAL', 'sendGlucoseFutureLowNotifications:', 'EVENT: Status', 'EVENT: Settings', 'EVENT: Bolus', 'Log.'];
    ...
    var treatCircles = chart().focus.selectAll('.treatment-dot').data(client.ddata.treatments.filter(function(treatment) {
      ...
      if (treatment.eventType.indexOf('Log.') === 0) {
	      return false
      }
      ...
      var notLoopSpam = enteredBy.indexOf('loop://') === -1 || _.isUndefined(_.find(loopSpam, function startsWith (spam) {
        ...
        // console.error(notes, spam, notes.indexOf(spam));
        return notes.indexOf(spam) === 0;
      }));

      return notCarbsOrInsulin && !treatment.duration && treatment.durationType !== 'indefinite' && notTempOrProfile && notOpenAPSSpam && notLoopSpam;
